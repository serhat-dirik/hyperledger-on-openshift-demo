...
for ORG in techpulse dataforge neuralpath; do
  ...
  # Install
  peer lifecycle chaincode install /work/${CC_NAME}.tar.gz 2>&1

  # Get package ID (same for all peers since same package)
  # Use head -1 to grab only the latest installed package
  if [ -z "$PACKAGE_ID" ]; then
    PACKAGE_ID=$(peer lifecycle chaincode queryinstalled 2>&1 | grep "${CC_NAME}_${CC_VERSION}" | sed -n 's/.*Package ID: \(.*\), Label:.*/\1/p' | head -1)

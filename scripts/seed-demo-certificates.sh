...
if [ -n "$TP_TOKEN" ]; then
    issue_cert "$API_TECHPULSE" "$TP_TOKEN" "TP-FSWD-001" "student01" "Alice Chen" "FSWD-101" "Full-Stack Web Dev" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_TECHPULSE" "$TP_TOKEN" "TP-FSWD-002" "student02" "Bob Martinez" "FSWD-101" "Full-Stack Web Dev" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_TECHPULSE" "$TP_TOKEN" "TP-CNM-001" "student01" "Alice Chen" "CNM-201" "Cloud-Native Microservices" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_TECHPULSE" "$TP_TOKEN" "TP-DSO-001" "student02" "Bob Martinez" "DSO-301" "DevSecOps Fundamentals" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_TECHPULSE" "$TP_TOKEN" "TP-CNM-002" "student01" "Alice Chen" "CNM-201" "Cloud-Native Microservices" "$ISSUE_DATE" "$EXPIRY_DATE"
...
if [ -n "$DF_TOKEN" ]; then
    issue_cert "$API_DATAFORGE" "$DF_TOKEN" "DF-PGA-001" "student03" "Carol Wang" "PGA-101" "PostgreSQL Administration" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_DATAFORGE" "$DF_TOKEN" "DF-DPE-001" "student04" "David Kim" "DPE-201" "Data Pipeline Engineering" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_DATAFORGE" "$DF_TOKEN" "DF-PGA-002" "student03" "Carol Wang" "PGA-101" "PostgreSQL Administration" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_DATAFORGE" "$DF_TOKEN" "DF-GDB-001" "student04" "David Kim" "GDB-301" "Graph Databases Masterclass" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_DATAFORGE" "$DF_TOKEN" "DF-DPE-002" "student03" "Carol Wang" "DPE-201" "Data Pipeline Engineering" "$ISSUE_DATE" "$EXPIRY_DATE"
...
if [ -n "$NP_TOKEN" ]; then
    issue_cert "$API_NEURALPATH" "$NP_TOKEN" "NP-AML-001" "student05" "Eva Patel" "AML-101" "Applied Machine Learning" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_NEURALPATH" "$NP_TOKEN" "NP-LFT-001" "student06" "Frank Liu" "LFT-201" "LLM Fine-Tuning Workshop" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_NEURALPATH" "$NP_TOKEN" "NP-CVP-001" "student05" "Eva Patel" "CVP-301" "Computer Vision Practicum" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_NEURALPATH" "$NP_TOKEN" "NP-AML-002" "student06" "Frank Liu" "AML-101" "Applied Machine Learning" "$ISSUE_DATE" "$EXPIRY_DATE"
    issue_cert "$API_NEURALPATH" "$NP_TOKEN" "NP-LFT-002" "student05" "Eva Patel" "LFT-201" "LLM Fine-Tuning Workshop" "$ISSUE_DATE" "$EXPIRY_DATE"

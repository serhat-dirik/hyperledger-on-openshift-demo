...
# ── Idempotency check ──────────────────────────────────────────
# If orderer0 already has the channel, skip orderer joins but ALWAYS
# run peer joins (a peer may have lost its PVC and need to rejoin).
EXISTING=$(osnadmin channel list -o orderer0:7053 \
  --ca-file /orderer-tls/ca.crt \
  --client-cert /orderer-tls/server.crt \
  --client-key /orderer-tls/server.key 2>/dev/null | grep "$CHANNEL_NAME" || true)
# ───────────────────────────────────────────────────────────────
...
if [ -z "$EXISTING" ]; then
  ...
  echo "[INFO] Channel '$CHANNEL_NAME' already exists on orderer0. Skipping orderer joins."
fi

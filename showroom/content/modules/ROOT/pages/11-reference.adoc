= Reference

== Demo Credentials

=== Platform Access

[cols="2,2,2,2"]
|===
| Service | Username | Password | Notes

| *OpenShift Console*
| `admin`
| (from RHDP provisioning email)
| {openshift_console_url}[Open^]

| *ArgoCD*
| —
| —
| Log in via *"Log in via OpenShift"* button using the OpenShift admin credentials above

| *Gitea* (Git repository)
| `gitea`
| `openshift`
| {gitea_url}[Open^]
|===

[TIP]
====
To retrieve the ArgoCD admin password directly:
[.console-input]
[source,bash]
----
oc get secret openshift-gitops-cluster -n openshift-gitops -o jsonpath='{.data.admin\.password}' | base64 -d && echo
----
====

=== Application Users

[cols="2,2,2,1"]
|===
| Role | Username | Password | Used For

| *TechPulse Admin*
| `admin@techpulse.demo`
| `admin`
| Course Manager UI

| *DataForge Admin*
| `admin@dataforge.demo`
| `admin`
| Course Manager UI

| *NeuralPath Admin*
| `admin@neuralpath.demo`
| `admin`
| Course Manager UI

| *TechPulse Student*
| `student01@techpulse.demo`
| `student`
| Cert Portal (transcript)

| *DataForge Student*
| `student03@dataforge.demo`
| `student`
| Cert Portal (transcript)

| *NeuralPath Student*
| `student05@neuralpath.demo`
| `student`
| Cert Portal (transcript)
|===

=== Infrastructure

[cols="2,2,2,1"]
|===
| Service | Username | Password | Used For

| *CouchDB* (per-org)
| `admin`
| `adminpw`
| State database (accessible via `oc exec`)

| *PostgreSQL* (per-org)
| `keycloak`
| `keycloak`
| Keycloak database
|===

== Environment URLs

=== User-Facing Applications

[cols="1,2"]
|===
| Component | URL

| Cert Portal (public verification)
| {cert_portal_url}[Open^]

| TechPulse Course Manager
| {techpulse_course_manager_url}[Open^]

| DataForge Course Manager
| {dataforge_course_manager_url}[Open^]

| NeuralPath Course Manager
| {neuralpath_course_manager_url}[Open^]
|===

=== Platform & Infrastructure

[cols="1,2"]
|===
| Component | URL

| OpenShift Console
| {openshift_console_url}[Open^]

| ArgoCD Dashboard
| {argocd_url}[Open^]

| Grafana Dashboards
| {grafana_url}[Open^]

| Gitea (Git Repository)
| {gitea_url}[Open^]
|===

=== Keycloak Identity Providers

[cols="1,2"]
|===
| Realm | URL

| Central (identity brokering)
| {central_keycloak_url}/admin[Open^]

| TechPulse
| {techpulse_keycloak_url}/admin[Open^]

| DataForge
| {dataforge_keycloak_url}/admin[Open^]

| NeuralPath
| {neuralpath_keycloak_url}/admin[Open^]
|===

Keycloak admin console credentials: `admin` / `admin` for all realms.

=== API Endpoints

[cols="1,2"]
|===
| API | Base URL

| TechPulse cert-admin-api
| {techpulse_cert_admin_api_url}

| DataForge cert-admin-api
| {dataforge_cert_admin_api_url}

| NeuralPath cert-admin-api
| {neuralpath_cert_admin_api_url}

| verify-api (central)
| {verify_api_url}
|===

== OpenShift CLI Access

To run `oc` commands from the Terminal tab, first log in to the cluster:

[.console-input]
[source,bash,subs="attributes"]
----
oc login {openshift_api_url} -u admin
----

[NOTE]
====
If you are running this demo on RHDP, the password is provided in your lab provisioning email. For BYO clusters, use the password you set during installation.
====

Verify the CertChain namespaces are healthy:

[.console-input]
[source,bash,subs="attributes"]
----
for ns in certchain certchain-techpulse certchain-dataforge certchain-neuralpath; do
  echo "=== $ns ==="
  oc get pods -n $ns --no-headers | grep -v Completed | awk '{print $1, $2, $3}'
done
----

== API Reference

=== cert-admin-api (per-org, requires `org-admin` role)

[cols="1,1,2"]
|===
| Method | Path | Description

| POST | `/api/v1/certificates` | Issue a new certificate
| GET | `/api/v1/certificates?page=0&size=20` | List org certificates (paginated)
| GET | `/api/v1/certificates/\{certId}` | Get single certificate
| PUT | `/api/v1/certificates/\{certId}` | Update mutable fields
| PUT | `/api/v1/certificates/\{certId}/revoke` | Revoke (irreversible)
| GET | `/api/v1/courses` | List course catalog
| GET | `/api/v1/dashboard/stats` | Certificate stats
|===

=== verify-api (central, public + JWT)

[cols="1,1,1,2"]
|===
| Method | Path | Auth | Description

| GET | `/api/v1/verify/\{certId}` | Public | Verify certificate status
| GET | `/api/v1/verify/\{certId}/qr` | Public | Generate QR code (PNG)
| GET | `/api/v1/verify/batch?ids=X,Y` | Public | Batch verify
| GET | `/api/v1/transcript` | JWT | Student's certificates
| GET | `/api/v1/transcript/\{certId}` | JWT | Single certificate
|===

== Namespace Layout

[cols="1,2"]
|===
| Namespace | Contents

| `certchain`
| Fabric CA, orderer0, cert-portal, verify-api, central Keycloak, Postgres, Grafana

| `certchain-techpulse`
| Peer, orderer, CouchDB, certcontract (CcaaS), Postgres, org Keycloak, cert-admin-api, course-manager-ui

| `certchain-dataforge`
| Same as techpulse

| `certchain-neuralpath`
| Same as techpulse

| `certchain-showroom`
| Antora lab guide, Gitea
|===

...
Each organization's peer maintains a full copy of the ledger and the CouchDB state database. This demo proves that when a peer goes down, the blockchain keeps working through other orgs, and the recovered peer syncs missed blocks.

=== Step 1: Record current state and take down the peer

[.console-input]
[source,bash,subs="attributes"]
----
# Record TechPulse cert count before the outage
echo "=== TechPulse cert count BEFORE peer down ==="
curl -sk "{techpulse_cert_admin_api_url}/api/v1/dashboard/stats" \
  -H "Authorization: Bearer $TOKEN" | python3 -m json.tool
----

Pause ArgoCD and kill the TechPulse peer:
...
=== Step 3: Bring TechPulse peer back and verify sync
...
Watch the peer sync missed blocks via gossip:

[.console-input]
[source,bash,subs="attributes"]
----
oc logs deployment/peer0 -n certchain-techpulse --tail=30 2>/dev/null | grep -i "gossip\|commit.*block\|Committed"
----

Look for: `Committed block [N]` entries — these are the blocks the peer missed while offline.
...
[source,bash,subs="attributes"]
----
# The cert issued through DataForge should now be on ALL peers (including TechPulse)
echo "=== Verifying cert issued during TechPulse outage ==="
curl -sk "{verify_api_url}/api/v1/verify/$PEER_TEST_ID" | python3 -m json.tool

echo ""
echo "=== TechPulse cert count AFTER recovery ==="
curl -sk "{techpulse_cert_admin_api_url}/api/v1/dashboard/stats" \
  -H "Authorization: Bearer $TOKEN" | python3 -m json.tool
----

The certificate issued through DataForge is now visible across the entire network — the peer synced the missed block.

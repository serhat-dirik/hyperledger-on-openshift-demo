...
| `{central_namespace}`
| Fabric CA, orderer0, cert-portal, verify-api, central Keycloak, Postgres, Grafana, certcontract (chaincode)

| `{central_namespace}-techpulse`
| peer0, orderer1, CouchDB, Postgres, org Keycloak, cert-admin-api, course-manager-ui

| `{central_namespace}-dataforge`
| peer0, orderer2, CouchDB, Postgres, org Keycloak, cert-admin-api, course-manager-ui

| `{central_namespace}-neuralpath`
| peer0, orderer3, CouchDB, Postgres, org Keycloak, cert-admin-api, course-manager-ui
|===
...
The Fabric CA in the central namespace issues all certificates. Each organization's MSP defines which identities are authorized to submit transactions on behalf of that organization.
